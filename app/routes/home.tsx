  ...
  <section className="py-12 bg-white">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-center max-w-7xl mx-auto">
      <div className="text-lg text-gray-800 leading-relaxed text-justify md:text-left space-y-6">
        ...
        <p className="font-medium">{t("home.about.p1")}</p>
        <p className="font-medium text-gray-700">{t("home.about.p2")}</p>
      </div>
...
const FeatureCard = ({ title, description, iconPath, isHtml = false }: FeatureCardProps) => (
  <div className="p-6 rounded-2xl transition-colors hover:bg-red-100">
    ...
    <h5 className="text-2xl font-semibold mb-4 text-gray-900">{title}</h5>
    <p className="text-gray-600 leading-relaxed">
      {isHtml ? <span dangerouslySetInnerHTML={{ __html: description }} /> : description}
    </p>
  </div>
);
...
const BestsellersCarousel = ({ t, bestsellers }: { t: any; bestsellers: Bestseller[] }) => {
  const [currentIndex, setCurrentIndex] = useState(0);

  const prevSlide = () => setCurrentIndex((prev) => (prev === 0 ? bestsellers.length - 1 : prev - 1));
  const nextSlide = () => setCurrentIndex((prev) => (prev === bestsellers.length - 1 ? 0 : prev + 1));
  ...
  return (
    <main className="grow pt-16">
      ...
      <div className="font-sans">
        <AboutSection t={t} />
        <FeaturesSection t={t} />
        <BestsellersCarousel t={t} bestsellers={bestsellers} />
      </div>
    </main>
  );
}