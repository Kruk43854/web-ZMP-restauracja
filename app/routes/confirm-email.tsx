...
const LoadingView = ({ t }: { t: any }) => (
  <>
    ...
    <h2 className="text-3xl font-bold font-fancy text-gray-800">
      {t("confirmEmail.loading_title")}
    </h2>
    <p className="text-gray-600">
      {t("confirmEmail.verifying_initial")}
    </p>
  </>
);
...
const ErrorView = ({ message, t }: { message: string; t: any }) => (
  <>
    ...
    <h2 className="text-3xl font-bold font-fancy text-red-600">
      {t("confirmEmail.error_title")}
    </h2>
...
const SuccessView = ({ message, t }: { message: string; t: any }) => (
  <>
    ...
    <h2 className="text-3xl font-bold font-fancy text-green-600">
      {t("confirmEmail.success_title")}
    </h2>
    <p className="text-gray-600">{message}</p>
  ...
  const verifyEmail = async (tokenValue: string) => {
    try {
      ...
      if (!response.ok || data?.success === false || data?.isSuccess === false) {
        const serverMsg = data?.message || data?.error || t("confirmEmail.error_default");
        const lowerCaseMsg = serverMsg.toLowerCase();

        // Obejście dla przypadku, gdy e-mail został już potwierdzony
        if (lowerCaseMsg.includes("already confirmed") || lowerCaseMsg.includes("already used")) {
          setStatus("success");
